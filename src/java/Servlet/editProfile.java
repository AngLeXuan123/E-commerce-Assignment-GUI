...
public class editProfile extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (errorCount == 0) {
                editAccount(pass, birthDate, email, phoneNumber, input, username);
    ...
    private void initializeJdbc() {
        try {
            ...
            pstmt = conn.prepareStatement("update account set password = ?, birthDate = ?, email = ?, phoneNumber = ?, photo = ? where id = ?");
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }

    private void editAccount(String pass, java.sql.Date birthDate, String email, String phoneNumber, InputStream photo, String username) throws SQLException {
